...
const import_lazyParsers = (app, ob)=> {
  class SheetElement extends ob.MarkdownRenderChild {
    constructor(el, source) {
      super(el); el.id = tableId
      this.tableHead = el.createEl('thead')
      this.tableBody = el.createEl('tbody')
  ...
  return {
    postParser,
    blockParser: (source, el, ctx)=> source && ctx.addChild(new SheetElement(el, source))
  }
}
